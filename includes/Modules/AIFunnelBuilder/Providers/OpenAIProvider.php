...
class OpenAIProvider implements AIProviderInterface {
	...
	public function __construct( AISettingsManager $settings ) {
		...
		$this->api_key  = $settings->get_api_key( 'openai' );
		$this->model    = $settings->get_model( 'openai' );
	}
	...
	public function generate( $prompt, $options = [] ) {
		if ( ! $this->is_configured() ) {
			return [
				'success' => false,
				'error'   => __( 'OpenAI API key is not configured.', 'woo-smart-automation' ),
			];
		}
		...
		if ( $response_code !== 200 ) {
			$error_message = isset( $data['error']['message'] ) 
				? $data['error']['message'] 
				: __( 'Unknown error from OpenAI API.', 'woo-smart-automation' );

			// Handle specific error codes
			if ( $response_code === 429 ) {
				$error_message = __( 'OpenAI API rate limit exceeded. Please try again later.', 'woo-smart-automation' );
			} elseif ( $response_code === 401 ) {
				$error_message = __( 'Invalid OpenAI API key. Please check your API key in settings.', 'woo-smart-automation' );
			} elseif ( $response_code === 402 ) {
				$error_message = __( 'OpenAI account has insufficient credits. Please add credits to your account.', 'woo-smart-automation' );
			}
		...
		if ( empty( $html ) ) {
			return [
				'success' => false,
				'error'   => __( 'OpenAI returned empty response. Please try again.', 'woo-smart-automation' ),
			];
		}
